...
def lrt_stops
  ...
  execute(<<-SQL)
    SELECT
      stops.id, stops.name
    FROM
      stops JOIN routes ON routes.stop_id = stops.id
    WHERE
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    ...
    JOIN
      stops AS transfer ON to_transfer.stop_id = trasnfer.id
    JOIN
      routes AS from_trasnfer ON transfer.id = from_transfer.stop_id
    JOIN
